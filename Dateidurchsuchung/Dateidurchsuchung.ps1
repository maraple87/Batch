...
#$name = Select-String -path $logdatei
...
$body =  "LOGDATEI: " + $name + " `n" +  "Server: al-sql-04`n" + "Pfad: \\al-sql-04\c$\Program Files\Microsoft SQL Server\MSRS13.MSSQLSERVER\Reporting Services\LogFiles\" + $name + "`n" + "---------------------------------------------------------`n" +" `n" +  "FEHLERMELDUNGEN: `n" + " `n" + " `n" + $body 
...
Send-MailMessage -to "martin.plewik@stimme.de", "thomas.mayenberger@stimme.de" -from "PowerShell <report@stimme.de>" -Subject "Auswertung Reportlogdateien" -body "$body" -SmtpServer $EmailServer 
}
#, "thomas.mayenberger@stimme.de"
